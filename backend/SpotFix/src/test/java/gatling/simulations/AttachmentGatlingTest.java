 ...
 */
public class AttachmentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all attachments").get("/api/attachments").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new attachment")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created attachment").get("${new_attachment_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created attachment").delete("${new_attachment_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Attachment entity").exec(scn);

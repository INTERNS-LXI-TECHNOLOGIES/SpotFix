 ...
 */
public class UserProfileGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all userProfiles").get("/api/user-profiles").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new userProfile")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created userProfile").get("${new_userProfile_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created userProfile").delete("${new_userProfile_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the UserProfile entity").exec(scn);

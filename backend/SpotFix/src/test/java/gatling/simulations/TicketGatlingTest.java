...
public class TicketGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all tickets").get("/api/tickets").headers(headersHttpAuthenticated).check(status().is(200)))
                        ...
                        .body(
                            StringBody(
                                "{" +
                                    ...
                                    ", \"expectedResolutionDate\": \"2020-01-01T00:00:00.000Z\"" +
                                    ", \"resolvedDate\": \"2020-01-01T00:00:00.000Z\"" +
                                    ", \"aiSummary\": null" +
                                    ", \"aiDuplicate\": null" +
                                    ", \"duplicateScore\": 0" +
                                    ", \"aiConfidence\": 0" +
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created ticket").get("${new_ticket_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created ticket").delete("${new_ticket_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

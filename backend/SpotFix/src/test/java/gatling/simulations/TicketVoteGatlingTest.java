...
public class TicketVoteGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all ticketVotes").get("/api/ticket-votes").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new ticketVote")
                        .post("/api/ticket-votes")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"voteType\": \"UPVOTE\"" + ", \"createdDate\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_ticketVote_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created ticketVote").get("${new_ticketVote_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created ticketVote").delete("${new_ticketVote_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TicketVote entity").exec(scn);

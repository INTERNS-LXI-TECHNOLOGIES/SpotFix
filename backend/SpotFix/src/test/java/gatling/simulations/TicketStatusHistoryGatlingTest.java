...
public class TicketStatusHistoryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all ticketStatusHistories")
                    .get("/api/ticket-status-histories")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new ticketStatusHistory")
                        .post("/api/ticket-status-histories")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                    "\"oldStatus\": \"OPEN\"" +
                                    ", \"newStatus\": \"OPEN\"" +
                                    ", \"changedDate\": \"2020-01-01T00:00:00.000Z\"" +
                                    "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_ticketStatusHistory_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created ticketStatusHistory").get("${new_ticketStatusHistory_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created ticketStatusHistory").delete("${new_ticketStatusHistory_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TicketStatusHistory entity").exec(scn);

...
public class WorkPlanGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all workPlans").get("/api/work-plans").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new workPlan")
                        .post("/api/work-plans")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                    "\"estimatedCost\": 0" +
                                    ", \"startedDate\": \"2020-01-01T00:00:00.000Z\"" +
                                    ", \"expectedCompletionDate\": \"2020-01-01T00:00:00.000Z\"" +
                                    ", \"actualCompletionDate\": \"2020-01-01T00:00:00.000Z\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_workPlan_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created workPlan").get("${new_workPlan_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created workPlan").delete("${new_workPlan_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the WorkPlan entity").exec(scn);

...
public interface LocationRepository extends ReactiveCrudRepository<Location, Long>, LocationRepositoryInternal {
    ...
    @Query("SELECT * FROM location entity WHERE entity.ward_id = :id")
    Flux<Location> findByWard(Long id);

    @Query("SELECT * FROM location entity WHERE entity.ward_id IS NULL")
    Flux<Location> findAllWhereWardIsNull();
